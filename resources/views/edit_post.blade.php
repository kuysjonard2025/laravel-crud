...
@section('content')
<form action="{{ route('posts.update', $post->id) }}" method="POST" class="mt-4 max-w-sm mx-auto">
    @if (session('success'))
    ...
    @if (session('error'))
        <div class="mb-4 p-4 bg-red-100 text-red-700 rounded-md text-center font-semibold">
            {{ session('error') }}
        </div>
    @endif
